...
export class FilmFormComponent implements OnInit, OnChanges {
  ...
  constructor(private filmService: FilmService, private imageService: ImageService, private location: Location) { }
  ...
  ngOnInit() {
    this.filmService.getGenres().subscribe((genres: Genre[]) => {
      this.filmGenres = genres;
    });
    this.filmService.getQualities().subscribe((qualities: Quality[]) => {
